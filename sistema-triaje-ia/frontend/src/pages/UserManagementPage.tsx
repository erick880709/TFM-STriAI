  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-[#164E63]">👥 Gestión de Usuarios</h1>
          <p className="text-sm text-[#64748B]">Administración de cuentas del sistema</p>
        </div>
      ...
      {!data?.length ? <EmptyState message="No hay usuarios registrados." /> : (
        <div className="overflow-x-auto">
          <table className="w-full bg-white border border-[#A5F3FC] rounded-lg text-sm">
            ...
            <tbody>
              {(data || []).map((u: UserInfo) => (
                <tr key={u.IdUsuario} className="border-b border-[#A5F3FC] hover:bg-[#ECFEFF]">
      ...
      {confirmDeactivate && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" role="dialog" aria-modal="true" aria-label="Confirmar desactivación">
          <div className="bg-white rounded-xl p-6 w-full max-w-sm space-y-4">
            ...
            <p className="text-sm text-[#64748B]">El usuario no podrá iniciar sesión. Esta acción se puede revertir reactivando al usuario.</p>
      ...
      {resetPassword && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" role="dialog" aria-modal="true" aria-label="Contraseña reseteada">
          <div className="bg-white rounded-xl p-6 w-full max-w-sm space-y-4">
            ...
            <p className="text-sm text-[#64748B]">La nueva contraseña temporal es:</p>
            <div className="bg-[#ECFEFF] rounded-lg p-3 font-mono text-lg text-center select-all">{resetPassword}</div>
            ...
            <div className="flex gap-3 pt-2">
              <button
                ...
                className="flex-1 py-2 border border-[#A5F3FC] rounded-lg text-sm hover:bg-[#ECFEFF]"
              >
                {resetCopied ? '✅ Copiado' : '📋 Copiar'}
              </button>
              <button onClick={() => setResetPassword(null)} className="flex-1 py-2 bg-[#164E63] text-white rounded-lg text-sm hover:bg-[#0A4C5C]">Cerrar</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-40" role="dialog" aria-modal="true" aria-label="Nuevo usuario">
          <div className="bg-white rounded-xl p-6 w-full max-w-md space-y-4">
            ...
            {(['username', 'password', 'email'] as const).map((f) => (
              <div key={f}>
                <label className="block text-xs font-medium text-[#64748B] mb-1 capitalize">{f}</label>
                <input
