...
export default function ModelManagementPage() {
  ...
  return (
    <div>
      ...
      {s && (
        <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm mb-6 ${s.modelo_cargado ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}>
        ...
        <div className="space-y-3">
          {diskModels.data?.map((m: DiskModel) => (
            <div key={m.version} className="bg-white border border-slate-200 rounded-lg p-4 flex items-center justify-between">
              <div>
                <p className="font-medium text-slate-800">{m.nombre} <span className="text-xs text-green-600">🟢 ACTIVO</span></p>
                <p className="text-xs text-slate-400">v{m.version} · {m.n_features} features · {m.tamano_mb} MB</p>
              </div>
              <span className="text-sm text-slate-500">F1: {m.f1_macro?.toFixed(4) || '?'}</span>
            </div>
          ))}
