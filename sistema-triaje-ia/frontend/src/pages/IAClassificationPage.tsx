    ...
    <div className="mt-4 space-y-2">
      {data.top_features?.slice(0, 10).map((f, i) => (
        <div key={i} className="flex items-center gap-2 text-sm">
          ...
          <span className="flex-1 text-[#164E63]">{f.feature}</span>
          <span className="text-[#64748B]">{f.importancia.toFixed(4)}</span>
        </div>
      ))}
      {data.fallback && (
...
export default function IAClassificationPage() {
  ...
  if (!triageId) {
    return (
      <div className="text-center py-12">
        <p className="text-[#64748B]">No hay un triaje activo. Inicia desde el registro de paciente.</p>
  ...
  return (
    <div className="max-w-3xl">
      ...
      <h1 className="text-2xl font-bold text-[#164E63] mb-1">🧠 Clasificación IA</h1>
      <p className="text-sm text-[#64748B] mb-6">Predicción del nivel de triaje por el modelo de Machine Learning</p>

      {!result && !predictMutation.isPending && (
        <div className="bg-white border border-[#A5F3FC] rounded-lg p-8 text-center">
          <p className="text-[#64748B] mb-4">Ejecuta la clasificación automática con IA usando los datos clínicos del paciente</p>
          <button
      ...
      {result && (
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-white border border-[#A5F3FC] rounded-lg p-6 text-center">
              <p className="text-sm text-[#64748B] mb-2">Nivel Predicho</p>
              <div
            ...
            <div className="bg-white border border-[#A5F3FC] rounded-lg p-6 md:col-span-2">
              <p className="text-sm text-[#64748B] mb-3">Probabilidades por Nivel</p>
              {NIVELES_TRIAGE.map((nivel) => {
                ...
                return (
                  <div key={nivel} className="flex items-center gap-2 mb-2">
                    ...
                    <div className="flex-1 bg-[#ECFEFF] rounded-full h-4 overflow-hidden">
                      <div
                        className="h-full rounded-full transition-all"
                        style={{ width: `${(prob * 100).toFixed(0)}%`, backgroundColor: NIVELES_COLORS[nivel] }}
                      />
                    </div>
                    <span className="text-xs text-[#64748B] w-12 text-right">{(prob * 100).toFixed(0)}%</span>
                  </div>
                )
              })}
            </div>
