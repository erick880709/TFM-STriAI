...
function KpiCard({ label, value, sub }: { label: string; value: string; sub?: string }) {
  return (
    <div className="bg-white border border-[#A5F3FC] rounded-lg p-4">
      <p className="text-xs text-[#64748B] uppercase tracking-wider">{label}</p>
      <p className="text-2xl font-bold text-[#164E63] mt-1">{value}</p>
...
export default function DashboardPage() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-[#164E63] mb-1">📊 Dashboard Operativo</h1>
      <p className="text-sm text-[#64748B] mb-6">Indicadores de desempeño del sistema de triaje</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white border border-[#A5F3FC] rounded-lg p-5">
          <h2 className="font-semibold text-[#164E63] mb-4">Distribución por Nivel IA</h2>
          {nivelData.length > 0 ? (
        ...
        <div className="bg-white border border-[#A5F3FC] rounded-lg p-5">
          <h2 className="font-semibold text-[#164E63] mb-4">Tendencia 7 Días</h2>
      ...
      <div className="mt-6 bg-white border border-[#A5F3FC] rounded-lg p-5">
        <h2 className="font-semibold text-[#164E63] mb-4">Desempeño del Modelo IA</h2>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            <div key={m.label} className="text-center p-3 bg-[#ECFEFF] rounded-lg">
              <p className="text-xs text-[#64748B] mb-1">{m.label}</p>
              <p className="text-xl font-bold text-[#164E63]" style={{fontFamily:'Lexend,system-ui,sans-serif'}}>{m.value}</p>
      ...
      {/* Concordancia */}
      <div className="mt-6 bg-white border border-[#A5F3FC] rounded-lg p-5">
        <h2 className="font-semibold text-[#164E63] mb-4">Concordancia IA vs. Profesional</h2>
        <div className="flex items-center gap-6 flex-wrap">
          <div className="text-center">
            ...
            <p className="text-xs text-[#64748B]">Global</p>
          </div>
          <div className="flex-1">
            {nivelData.map(n => (
              <div key={n.nivel} className="flex items-center gap-2 text-sm mb-1">
                ...
                <div className="flex-1 bg-[#ECFEFF] rounded-full h-2 overflow-hidden">
                ...
                <span className="text-xs text-[#64748B] w-8 text-right">{n.triajes}</span>
              </div>
            ))}
          </div>
        </div>
