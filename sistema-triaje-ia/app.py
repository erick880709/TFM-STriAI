...
Aplicación principal (Streamlit) — v2 optimizada para rendimiento.

Optimizaciones aplicadas:
  - @st.cache_resource para AuthService, configuración e init de BD
  - @st.fragment en sidebar (evita re-renderizados cruzados con el contenido)
  - Lazy imports en router de páginas (solo carga la página activa)
  - Sin servicios en st.session_state (todo vía cached.py)
"""
...
# ---------------------------------------------------------------------------
# PATH — una sola vez (efímero, no afecta rendimiento)
# ---------------------------------------------------------------------------
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))


# ---------------------------------------------------------------------------
# INICIALIZACIÓN CACHEADA — se ejecuta UNA sola vez por sesión
# ---------------------------------------------------------------------------
@st.cache_resource
def _init_app():
    """Inicializa configuración, BD y AuthService. Se llama una sola vez."""
...
# Retrocompatibilidad: las páginas usan st.session_state.db_path
if "db_path" not in st.session_state:
    st.session_state.db_path = db_path
if "app_config" not in st.session_state:
    st.session_state.app_config = _app["config"]


# ---------------------------------------------------------------------------
# SIDEBAR — Navegación y sesión.
# NOTA: NO se usa @st.fragment aquí porque los botones de navegación DEBEN
#       disparar un rerun completo para que el router cargue la nueva página.
#       La optimización viene de @st.cache_resource en _init_app() y de los
#       imports lazy en cached.py (que evitan cargar torch/shap en páginas
#       que no usan IA).
# ---------------------------------------------------------------------------
def _render_sidebar():
    """Sidebar con navegación. Solo se re-renderiza cuando cambia la sesión."""
    with st.sidebar:
        st.title("🏥 Triaje IA")
        ...
        if "RegistroPaciente" in roles_permitidos:
            if st.button("📝 Registrar Paciente", use_container_width=True, key="nav_reg_pac"):
                st.session_state.page = "registro_paciente"
        if "SignosVitales" in roles_permitidos:
            if st.button("💓 Signos Vitales", use_container_width=True, key="nav_signos"):
                st.session_state.page = "signos_vitales"
        if "EvaluacionClinica" in roles_permitidos:
            if st.button("🩺 Evaluación Clínica", use_container_width=True, key="nav_eval"):
                st.session_state.page = "evaluacion_clinica"
        if "ClasificacionIA" in roles_permitidos:
            if st.button("🧠 Clasificación IA", use_container_width=True, key="nav_ia"):
                st.session_state.page = "clasificacion_ia"
        if "ClasificacionIA" in roles_permitidos:
            if st.button("✅ Validación y Cierre", use_container_width=True, key="nav_val"):
        ...
        if "GestionModelos" in roles_permitidos:
            if st.button("⚙️ Gestión Modelos", use_container_width=True, key="nav_modelos"):
                st.session_state.page = "gestion_modelos"
        if "ComparacionModelos" in roles_permitidos:
            if st.button("🔬 Comparar Modelos", use_container_width=True, key="nav_comp"):
                st.session_state.page = "comparacion_modelos"
        if "Auditoria" in roles_permitidos:
            if st.button("🔍 Auditoría", use_container_width=True, key="nav_audit"):
                st.session_state.page = "auditoria"

        st.markdown("---")
        if "GestionUsuarios" in roles_permitidos:
            if st.button("👥 Gestión Usuarios", use_container_width=True, key="nav_users"):
                st.session_state.page = "gestion_usuarios"
            if st.button("📝 Control de Cambios", use_container_width=True, key="nav_cc"):
                st.session_state.page = "control_cambios"

        st.markdown("---")
        st.subheader("📜 Consultas")
        if st.button("📜 Histórico del Paciente", use_container_width=True, key="nav_hist"):
...
# ---------------------------------------------------------------------------
# ROUTER DE PÁGINAS — imports lazy (solo carga la página activa)
# ---------------------------------------------------------------------------
def _route_page():
    """Router central. Cada import ocurre solo si la página está activa."""
